<?php
session_start();
$_SESSION['later'] = time();
$lapseTime = $_SESSION['later'] - $_SESSION['now'];
$_SESSION['lapseTime'] = $lapseTime;
//echo "Form submitted after $lapseTime";

$host = 'localhost';
$username = 'onyx5';
$password = 'shalom';
$db_name = 'biblesco';
$conn = mysqli_connect($host, $username, $password, $db_name);
if(!$conn) 
{
  die("Connection failed: " . mysqli_connect_error());
}
  
  $query = "SELECT student_id FROM biblestudent where user_name = '" . $_SESSION['user'] . "' && pass_word = '" . $_SESSION['pass'] ."'";
            
  $result = mysqli_query($conn,$query) ;	
  $row = mysqli_fetch_assoc($result);
  $student_id = $row['student_id'];
  
  $ques = htmlspecialchars($_POST['ques']);
  $answer = htmlspecialchars($_POST['answer']);
  
  $insert = 'INSERT INTO answers(student_id, question, answer, created, answer_time)
             VALUES( "' . $student_id .'", "' . $ques .'", "' . $answer . '", NOW(), "' . $lapseTime .'")'; 
  $inresult = mysqli_query($conn,$insert);			 
  
?>
<!DOCTYPE html>
<html>
	<head>
		<title> Website </title>
		<meta charset="utf-8">
    	<meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
		<link rel="stylesheet" href="css/bootstrap.min.css">
         <link rel="stylesheet" href="style/myboot.css">
		<!--<link href="css/bootstrap.css" rel="stylesheet" type="text/css">-->
          <script src="js/jquery-3.3.1.min.js"></script>
          <script src="js/biblereading.js"></script>
		
	</head>
	<body>
		
		<section style="width:91%; margin:auto;">
			<div class="container">
				<div class="row">
					<div class="col-sm-12" style="height: 130px;">
					<img src="images/biblegurufinal.png" style="margin-top:0px; position:relative; "/>
					<a href="index.php" class="backToHome">Back to home page</a>	
					</div>
				</div>
				
				<div class="row">
					<div class="col-sm-12" style="height:500px;">
					<span class="regres"><?php echo "Thank you for supplying the answer to the question.<br/> 
                                Now you can go <a href='index.php'>back</a> to the home page to read more...";
								 
					?></span>
						
					</div>
				</div>
				
				<div class="row">
					<div class="col-sm-12" style="background-color:#b85c90; padding:7px; text-align:center;">
						<!--<h1>FOOTER</h1>-->
						<h5 style="color:white;">&copy; copyright 2019 Onyx Creative solutions</h5>
					</div>
				</div>
			</div>
		</section>


		
	</body>
</html>